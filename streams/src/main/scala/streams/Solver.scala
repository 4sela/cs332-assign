  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) {
      ...
      val newNeighbors = newNeighborsOnly(
        neighborsWithHistory(current._1, current._2),
        explored
      )

      val updatedStream = rest ++ newNeighbors
      val updatedExplored = explored ++ newNeighbors.map(_._1)

      current #:: from(updatedStream, updatedExplored)
    }
  }
